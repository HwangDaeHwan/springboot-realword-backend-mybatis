...
import com.realworld.backend.application.data.ArticleDataList;
import com.realworld.backend.application.data.ArticleFavoriteCount;
import com.realworld.backend.core.user.User;
import com.realworld.backend.infrastructure.mybatis.readservice.ArticleFavoritesReadService;
import com.realworld.backend.infrastructure.mybatis.readservice.ArticleReadService;
...
public class ArticleQueryService {
	...
	public CursorPager<ArticleData> findRecentArticlesWithCursor(String tag, String author, String favoritedBy,
	...
	private void setIsFollowingAuthor(List<ArticleData> articles, User currentUser) {
		Set<String> followingAuthors = userRelationshipQueryService.followingAuthors(currentUser.getId(),
				articles.stream().map(articleData1 -> articleData1.getProfileData().getId()).collect(Collectors.toList()));
	...
	private void setFavoriteCount(List<ArticleData> articles) {
		List<ArticleFavoriteCount> favoritesCounts = articleFavoritesReadService
				.articlesFavoriteCount(articles.stream().map(ArticleData::getId).collect(Collectors.toList()));
	...
	private void setIsFavorite(List<ArticleData> articles, User currentUser) {
		Set<String> favoritedArticles = articleFavoritesReadService.userFavorites(
				articles.stream().map(articleData -> articleData.getId()).collect(Collectors.toList()), currentUser);
